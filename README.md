...
```
Далее в зависимости от настроек вашего терминала может, например, открыться консольный редактор умолчания vim, вам будет предложеное заполнить шаблон очередной миграции конкретными SQL запросами. Можно сделать это прямо там, а можно сохранить шаблон без иземенений (`:w! :q!`) и отредактировать создавшийся файл позже.
