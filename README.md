...
3. Написать API "переднего" back'а, который был бы посредником между front end'ом и API этого проекта. Это нужно для того, чтобы front end'у не приходилось делать запросы в разные API для получение полной информации по тем же пользователям.
